...
{
    public static void Initialize(in IncrementalGeneratorInitializationContext context)
    {
        ...
        var interceptorsToGenerate = context.SyntaxProvider
            .CreateSyntaxProvider(
                predicate: static (node, _) => node is InvocationExpressionSyntax { Expression: MemberAccessExpressionSyntax { Name.Identifier.ValueText: Constants.Contracts.ISender_SendMethod } },
    ...
    static RequestHandlerToGenerate? HandlerTransform(in GeneratorSyntaxContext context, in CancellationToken ct)
    {
        ...
        foreach (var interfaceSymbol in symbol.AllInterfaces)
        {
            ...
            {
                return new RequestHandlerToGenerate(symbol, interfaceSymbol);
            }
        }
        return null;
        ...
        {
            Name: Constants.Requests.IRequestHandler,
    ...
    static RequestInterceptorToGenerate? InterceptorTransform(in GeneratorSyntaxContext context, in CancellationToken ct)
    {
        ...
        if (invocation.ArgumentList.Arguments.Count is 1 or 2
            ...
            && targetOperation.TargetMethod is { Name: Constants.Contracts.ISender_SendMethod, TypeArguments.Length: 1, Parameters.Length: 2 } targetMethod
            ...
            && context.SemanticModel.GetInterceptableLocation(invocation) is { } location)
        {
            ...
            if (type.TypeKind is TypeKind.Interface)
                sender = $"global::{Constants.Contracts.ISenderFull}";

            // Return the location details and the fully quialified type details.
            return new RequestInterceptorToGenerate(location, sender, request, response);
        }
        ...
        {
            Name: Constants.Contracts.ISender,
